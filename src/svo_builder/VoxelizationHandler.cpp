...
void voxelizeAndBuildSVOnePartition(
	TripInfo4D& trianglePartition_info, size_t currentPartitionNumber,
	float unitlength, size_t &nfilled, vector<uint64_t> &data,
	float sparseness_limit, uint64_t morton_part, char* voxels,
	OctreeBuilder &builder, size_t input_buffersize)
{
	if (trianglePartition_info.nbOfTrianglesPerPartition[currentPartitionNumber] == 0) { return; } // skip partition if it contains no triangles

																				// VOXELIZATION

	cout << "Voxelizing partition " << currentPartitionNumber << " ..." << endl;
	// morton codes for this partition
	uint64_t morton_startcode = currentPartitionNumber * morton_part;
	uint64_t morton_endcode = (currentPartitionNumber + 1) * morton_part;
	// open file to read triangles

	string part_data_filename = trianglePartition_info.base_filename + string("_") + val_to_string(currentPartitionNumber) + string(".tripdata");
	TriReader reader = TriReader(part_data_filename, trianglePartition_info.nbOfTrianglesPerPartition[currentPartitionNumber], min(trianglePartition_info.nbOfTrianglesPerPartition[currentPartitionNumber], input_buffersize));
	if (verbose) { cout << "  reading " << trianglePartition_info.nbOfTrianglesPerPartition[currentPartitionNumber] << " triangles from " << part_data_filename << endl; }
	...
	// build SVO
	cout << "Building SVO for partition " << currentPartitionNumber << " ..." << endl;
